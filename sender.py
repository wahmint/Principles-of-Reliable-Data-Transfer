...
class RDTSender:
    ...
    def rdt_send(self, process_buffer):
        ...
        for data in process_buffer:
            ...
            clone_packet= RDTSender.clone_packet(pkt)
            while RDTSender.is_corrupted(reply) or not RDTSender.is_expected_seq(reply,self.sequence):
                print('Sender recieved:',reply)
                reply = RDTSender.clone_packet(clone_packet)
                reply = self.net_srv.udt_send(reply)
                print('network_layer: corruption occurred',pkt)
            #if not RDTSender.is_corrupted(reply) or RDTSender.is_expected_seq(reply,self.sequence):
            if reply:
                print('Sender recieved:',reply)
                if self.sequence == '0':
                    self.sequence = '1'
